...
In the case of the plain `vagrant up` option, a VM will be brought up and configured in the current directory. The application is accessible on the local machine from a Web browser at IP address 192.168.0.2.

When `vagrant up` finishes with no failures, the server is accessible at this URL:
...
Several ports in the running VM are made accessible on the local machine. These ports are as follows:
...
this URL: `http://192.168.0.2:8983/solr`.
